...
const NavigationBar = () => {
    ...
    return (
        <>
            {!user.email ? (<Navbar style={{ backgroundColor: "#e6fcff" }}>
                <Container className="navigation__bar__main">
                    <Navbar.Brand href="/" style={{ fontSize: "1.6rem", fontWeight: "600", color: "black" }}>Medi Care</Navbar.Brand>
                    <div className="navigation__links">
                        <Link style={{color:"black"}} className="login__link" to="/login?show=true">Login</Link>
                        <Link style={{color:"black"}} className="register__link" to="/register?show=true">Register</Link>
                    </div>
                </Container>
            </Navbar>) : (<Navbar style={{ backgroundColor: "#e6fcff" }}>
                <Container>
                    <Navbar.Brand href="/home" style={{ fontSize: "1.6rem", fontWeight: "600", color: "black" }}>Medi Care</Navbar.Brand>
                    <div>
                        {user.userType === "patient" && <Link to="/myAppointments" style={{color:"black"}}  className="login__link" >My Appointments</Link>}
                        <Link to="/" className="register__link" style={{color:"black"}} onClick={handleLogout}>Logout</Link>
                    </div>
